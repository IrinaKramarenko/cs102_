...
def update_ref(
    gitdir: pathlib.Path, ref: tp.Union[str, pathlib.Path], new_value
) -> None:
    # PUT YOUR CODE HERE
...
def symbolic_ref(gitdir: pathlib.Path, name, ref) -> None:
    # PUT YOUR CODE HERE
    with open(gitdir / name, "w") as file:
        file.write("ref: " + ref)


def ref_resolve(gitdir: pathlib.Path, refname) -> str:
