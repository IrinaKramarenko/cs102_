...
def group(values: List[str], num: int) -> List[List[str]]:
    """
    ...
    [[1, 2, 3], [4, 5, 6], [7, 8, 9]]
    """
    group1 = [["0"] * num for i in range(num)]
    count = 0
    for i in range(num):
        for j in range(num):
            group1[i][j] = values[count]
            count += 1
    return group1
...
def check_solution(solution: List[List[str]]) -> bool:
    ...
    for i in range(len(solution)):
        for j in range(len(solution[i])):
            if (
                set(get_row(solution, (i, j))) != res
                or set(get_col(solution, (i, j))) != res
                or set(get_block(solution, (i, j))) != res
            ):
                return False
    return True
